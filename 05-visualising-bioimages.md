...
earlier is an example. In our mitochondrial image, the data range from 0 to 0.68
...
You will see in later episodes, there are images that use only 6% of the total. 
...
Another way to visualise the same data is to look at the shape of the distribution of the data, 
disregarding any spatial location information.
Histograms show us the pixel intensity distributions within an
image, leaving out the location information. 
...
In R, we could change the pixel values if we assig the new object to the same initial object. 
...
Let's compare the original and stretched image histograms:
...
range. The new image objects have different histograms, 
*the image data has changed!*

Importantly, it does not create new biological information.
...
```text
Pixel Values in the image -> Display Mapping Function -> Visible Image on the screen
```
...
A lookup table (LUT) defines how pixel intensities from the image are displayed on the screen. It is the mapping 
